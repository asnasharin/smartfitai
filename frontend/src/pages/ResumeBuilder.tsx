...
const ResumeBuilder = () => {
  ...
  const generateResumeContent = async () => {
    ...
    try {
      ...
      if (response.status === 201 && response.data.generatedText && response.data.resumeId) {
  setGeneratedText(response.data.generatedText);
  setGeneratedResumeId(response.data.resumeId);
  ...
  if (!generatedResumeId) {
    toast.error("No generated resume ID found.");
    return;
  }

  try {
    const response = await apiClient.get(
      `/api/builder/download/${generatedResumeId}`,
      { responseType: "blob" }
    );
